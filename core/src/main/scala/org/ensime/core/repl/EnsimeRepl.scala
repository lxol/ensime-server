...
/**
 * An `EnsimeRepl` is a simple Finite State Machine with 4 states based on
...
object EnsimeRepl {
  ...
  /**
   * Almost a `Listener`. Status calls are dispatched on the `EnsimeRepl` `Future`'s thread,
  ...
  trait Client {
    ...
    /**
     * Called when something is thrown. The `EnsimeRepl` `Future` responds to any exception
  ...
  trait Builder {
    /** Called by `EnsimeRepl` constructor, calls its `Worker.start`. */
    def constructed(a: EnsimeRepl): Unit = {}
 ...
 * previous lines, each `EnsimeRepl` should be used only by a single entity.
 ...
 */
class EnsimeRepl(client: Client, builder: Builder) {
