...
//import org.scalaide.core.internal.repl.EnsimeRepl
//import org.scalaide.core.internal.repl.EnsimeRepl._

import EnsimeReplTest._
import EnsimeRepl._
...
// EnsimeRepl source code file to there.

// ----------

// capitalization is important...
// in the EnsimeRepl public API:
...
class EnsimeReplSpec extends EnsimeSpec
    with EnsimeReplTestUtils {
  ...
  it should "do something that I don't understand yet" in {
  ...
  // // next use multiple EnsimeRepls in parallel to test the Future stuff ...
...
object EnsimeReplTest {
  import TestFutureUtil._
  ...
  case class InOut(in: String, out: (Int => String)) {
    def execAdded = EnsimeReplTest.execAdded(in)
    def DoingFailed = EnsimeReplTest.DoingFailed(in)
  ...
  class Recorder(val expected: Seq[Expect]) {
    ...
    val tested = new EnsimeRepl(client, TestBuilder)
...
trait EnsimeReplTestUtils extends Assertions {
  import scala.concurrent.ExecutionContext.Implicits.global
  import scala.concurrent.duration._
  val DefaultTimeout = 1000 millis
